...
- [ ] 4.1 先寫 spec：以小型樣本 `.pptx`（純文字 / 含圖片 / 含表格 / 含未支援元素）驗證解析與輸出
- [ ] 4.2 實作 `.pptx` 解析（jszip + fast-xml-parser）：投影片尺寸、頁序、形狀位置（EMU→百分比）
- [ ] 4.3 實作文字還原（段落/run/樣式）與表格還原
- [ ] 4.4 實作圖片擷取（rels→media→data URI 內嵌；不支援格式占位）
- [ ] 4.5 實作 HTML 輸出（長寬比鎖定容器 + 百分比定位 + `cqw` 字級，不用 transform）
- [ ] 4.6 實作準確率計算（涵蓋率 / 文字 / 圖片 / 加權整體）與每頁明細、對照中介結構
- [ ] 4.7 實作單檔容錯（解析例外 → 標記 failed + 原因）；所有 4.x spec 綠燈
