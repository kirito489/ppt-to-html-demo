...
- [x] 9.1 新增 `routes/articles` 路由群組與 nav 項目（文章列表、轉換紀錄）
- [x] 9.2 文章列表頁（標題/來源檔名/頁數/整體準確率/狀態 + 進入詳情 + 「立即抓取轉換」按鈕）
- [x] 9.3 文章檢視頁：不跑版自縮放 HTML 預覽容器（對應引擎輸出樣式）
- [x] 9.4 準確率儀表元件（整體 + 三項分類 + 每頁明細）
- [x] 9.5 來源元素對照面板（轉換結果 vs 來源元素清單，標記未還原）
- [x] 9.6 轉換紀錄頁（批次列表）
...
- [x] 11.1 放置數個樣本 `.pptx` 至 `storage/incoming`，建立 `smoke-test.md`（curl 範例）
- [x] 11.2 跑手動觸發與排程，驗證：列表→檢視（不跑版）→準確率→對照→來源已依策略清除
- [x] 11.3 確認文字與圖片內容於對照面板逐條正確
