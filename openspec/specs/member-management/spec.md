...
定義後端「會員管理」API 行為。本 spec 初版聚焦在 `GET /api/members` 列表 endpoint 的
篩選與分頁參數，未來會員 CRUD 行為（get / create / update / delete）皆收斂於此 capability。
與 `member-role-options-api` 區隔——後者只描述角色 Combobox 用的 endpoint。
...
- **WHEN** 使用者打 `GET /api/members?status=foo`
