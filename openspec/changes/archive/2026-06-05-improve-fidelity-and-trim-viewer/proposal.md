...
- **下載簡報 HTML 依投影片實際長寬比**：原寫死 16:9，遇非 16:9（如正方形）會讓整頁超出視窗被裁；改依 section 的 aspect-ratio 等比塞滿、不裁切。
...
- `converted-article-ui`: 修改不跑版預覽（移除捲動模式與模擬寬度切換，預覽僅保留翻頁）、HTML 匯出（移除複製 HTML）與下載簡報 HTML（依投影片實際長寬比，不裁切）。
